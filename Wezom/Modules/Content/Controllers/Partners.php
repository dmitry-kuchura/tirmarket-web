...
use Wezom\Modules\Content\Models\Partners AS Model;

class Partners extends Base
{

    public $tpl_folder = 'Content/Partners';
    public $limit;

    function before()
    {
        parent::before();
        $this->_seo['h1'] = __('Партнеры');
        $this->_seo['title'] = __('Партнеры');
        $this->setBreadcrumbs(__('Партнеры'), 'wezom/' . Route::controller() . '/index');
